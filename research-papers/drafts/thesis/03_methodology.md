...
A point forecast alone understates what a farmer-facing tool needs to communicate: how much uncertainty surrounds a given number. Rather than design a separate interval mechanism per model family — which would require quantile regression for the tree-based models, Monte Carlo dropout or a quantile loss for the LSTM, and would leave the two hybrids without a well-defined combination — this study uses a single **empirical, residual-based interval** applied uniformly across all seven families (`src.evaluation.metrics.prediction_interval`).

For each (vegetable, model family) pair, the residuals (actual − predicted) from all five walk-forward CV folds are pooled into one out-of-sample residual sample. At a configured confidence level `c` (0.8 by default, `forecasting.interval_confidence` in `configs/config.yaml`), the empirical (1−c)/2 and (1+c)/2 percentiles of that pooled residual sample are taken as a lower and upper offset, which are then added to each holdout-period point forecast to produce `predicted_lower` and `predicted_upper`. Because the residuals used to calibrate the interval come from the CV folds — data that precedes the holdout window — and the interval is then evaluated against the holdout's own actual prices, the holdout also functions as a genuine out-of-sample test of the interval's calibration, not merely of the point forecast's accuracy.

Calibration is quantified as **empirical coverage** (`src.evaluation.metrics.interval_coverage`): the fraction of holdout actual prices that fall within `[predicted_lower, predicted_upper]`. Comparing this against the nominal confidence level `c` indicates whether the interval is well-calibrated (coverage ≈ c), too narrow (coverage < c — residual variability understated), or unnecessarily wide (coverage > c). This coverage figure is reported per vegetable/model family alongside MAE/RMSE/MAPE/R² in `results/metrics/all_results.csv` and Chapter 4's results.

This design deliberately trades away model-specific sophistication (e.g. SARIMAX's own analytical confidence interval via its state-space formulation) for a method that is simple, uniform across heterogeneous model families, and reuses evaluation machinery (the CV folds) that the pipeline already computes for every model — consistent with this project's general preference for methodology that is defensible and reproducible over marginal per-model refinement. A known limitation, noted for completeness: pooling residuals equally across CV folds of different (expanding) training-window sizes implicitly treats early-fold residuals — from models fit on less data than the final production model ever sees — as equally representative as later-fold residuals; this is a standard simplification in residual-bootstrapping approaches, but means the reported interval width is not perfectly matched to the production model's true residual variance.
