...
The second is a **trust and usability gap**. Even where accurate forecasting models exist, they remain research artifacts: outputs are numbers in a spreadsheet or a research paper, not something a farmer or trader can act on. SHAP-based explanation and LLM-based advisory generation each now have a literature of their own (Chapter 2, Section 2.9) — so the gap is no longer that these techniques are unproven, but that they exist as two separate threads that have never been connected to each other, or to a Sri Lankan vegetable-price forecast specifically. None of the forecasting systems reviewed in Chapter 2 — Sri Lankan or otherwise — pair a price prediction with a SHAP explanation *and* translate that explanation into a plain-language recommendation in one pipeline. A farmer given a number with no explanation has little more basis for trust than a farmer given no forecast at all.
