...
- `GET /predictions/{vegetable}/future?model=best|<family>&start_date=&end_date=` — every not-yet-resolved week (`actual_price IS NULL`) for a vegetable, oldest first — the dedicated way to read "the upcoming forecast" as a trajectory rather than filtering the general list endpoint. `start_date`/`end_date` optionally narrow the range further. Empty list (not 404) if `scripts/predict_future.py` hasn't been run yet
- `GET /predictions?vegetable=&model=&year=&start_date=&end_date=&limit=&offset=` — filtered/paginated forecast history, including by date range. **If none of `year`/`start_date`/`end_date` are given, defaults to `forecast_date >= today`** (today's forecast plus everything future) rather than dumping the full history — pass an explicit `start_date` to see past forecasts too. This default only applies when all three are omitted; giving any one of them (even alone) disables it entirely.
